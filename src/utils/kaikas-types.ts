...
export interface Kaikas extends InjectedProviders {
  on?: (...args: any[]) => void
   ...
   */
  request(
    args: { method: 'web3_clientVersion' | 'klay_clientVersion' } | { method: 'klay_chainId' | 'eth_chainId' }
  ): Promise<string>
   ...
   */
  request(args: {
    method: 'klay_sign' | 'eth_sign'
    // params: [message: Bytes | string, account: Bytes | string, password: Bytes | string]
    params: [account: Bytes | string, message: Bytes | string]
  }): Promise<string>
  ...
  request(
    args:
      | {
          ...
          params: [{ klay_accounts: Record<string, any> }]
        }
      | {
  ...
  request(
    args:
      | {
          ...
          params: AddKlaytnChainParameter[]
        }
      | {
...
type WalletPermission = {
  ...
  parentCapability: 'klay_accounts' | string
}
...
export type ConnectorOptions = Pick<InjectedConnectorOptions, 'shimChainChangedDisconnect' | 'shimDisconnect'> & {
  /**
   * While "disconnected" with `shimDisconnect`, allows user to select a different kaikas account (than the currently connected account) when trying to connect.
   */
